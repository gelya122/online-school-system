...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260404181627_SyncSchemaPaymentMethod20260404'
)
BEGIN
    ...
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[payment]') AND [c].[name] = N'payment_method');
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[app_order]') AND [c].[name] = N'payment_method');
    IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [app_order] DROP CONSTRAINT [' + @var1 + '];');
    ALTER TABLE [app_order] DROP COLUMN [payment_method];
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[trial_application]') AND [c].[name] = N'selected_subjects');
    IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [trial_application] DROP CONSTRAINT [' + @var2 + '];');
    ALTER TABLE [trial_application] ALTER COLUMN [selected_subjects] nvarchar(500) NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[trial_application]') AND [c].[name] = N'last_name');
    IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [trial_application] DROP CONSTRAINT [' + @var3 + '];');
    ALTER TABLE [trial_application] ALTER COLUMN [last_name] nvarchar(100) NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[trial_application]') AND [c].[name] = N'class_number');
    IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [trial_application] DROP CONSTRAINT [' + @var4 + '];');
    ALTER TABLE [trial_application] ALTER COLUMN [class_number] int NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[school_setting]') AND [c].[name] = N'terms_of_use_url');
    IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [school_setting] DROP CONSTRAINT [' + @var5 + '];');
    ALTER TABLE [school_setting] ALTER COLUMN [terms_of_use_url] nvarchar(500) NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[school_setting]') AND [c].[name] = N'privacy_policy_url');
    IF @var6 IS NOT NULL EXEC(N'ALTER TABLE [school_setting] DROP CONSTRAINT [' + @var6 + '];');
    ALTER TABLE [school_setting] ALTER COLUMN [privacy_policy_url] nvarchar(500) NULL;
END;
...
BEGIN
    ALTER TABLE [payment] ADD [method_id] int NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[discount_type]') AND [c].[name] = N'is_active');
    IF @var7 IS NOT NULL EXEC(N'ALTER TABLE [discount_type] DROP CONSTRAINT [' + @var7 + '];');
    ALTER TABLE [discount_type] ADD DEFAULT CAST(1 AS bit) FOR [is_active];
END;
...
BEGIN
    ALTER TABLE [app_order] ADD [method_id] int NULL;
END;
...
BEGIN
    CREATE TABLE [payment_method] (
        [method_id] int NOT NULL IDENTITY,
...
BEGIN
    EXEC(N'ALTER TABLE [review] ADD CONSTRAINT [CK_review_rating] CHECK ([rating] IS NULL OR ([rating] >= 1 AND [rating] <= 5))');
END;
...
BEGIN
    CREATE INDEX [IX_promo_code_type_id] ON [promo_code] ([type_id]);
END;
...
BEGIN
    CREATE INDEX [IX_payment_method_id] ON [payment] ([method_id]);
END;
...
BEGIN
    CREATE INDEX [IX_app_order_method_id] ON [app_order] ([method_id]);
END;
...
BEGIN
    ALTER TABLE [app_order] ADD CONSTRAINT [FK_app_order_payment_method] FOREIGN KEY ([method_id]) REFERENCES [payment_method] ([method_id]);
END;
...
BEGIN
    ALTER TABLE [payment] ADD CONSTRAINT [FK_payment_payment_method] FOREIGN KEY ([method_id]) REFERENCES [payment_method] ([method_id]);
END;
...
BEGIN
    ALTER TABLE [promo_code] ADD CONSTRAINT [FK_promo_code_discount_type] FOREIGN KEY ([type_id]) REFERENCES [discount_type] ([type_id]);
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260404181627_SyncSchemaPaymentMethod20260404', N'9.0.0');
END;
